...
ALTER TABLE BoaIniciativa.Doacao ALTER COLUMN doadorCPF SET DEFAULT 00000000000;
...
ALTER TABLE ONLY BoaIniciativa.Doacao
    ADD CONSTRAINT FK_doadorCPF FOREIGN KEY (doadorCPF) REFERENCES BoaIniciativa.Usuario(CPF) ON UPDATE CASCADE ON DELETE SET default;
CREATE INDEX FKI_idCampanha ON BoaIniciativa.Doacao USING btree (idCampanha);
ALTER TABLE ONLY BoaIniciativa.Doacao
    ADD CONSTRAINT FK_idCampanha FOREIGN KEY (idCampanha) REFERENCES BoaIniciativa.Campanha(idCampanha) ON UPDATE CASCADE ON DELETE CASCADE;
...
CREATE TABLE BoaIniciativa.Material (
    ...
    CONSTRAINT nomeMaterial UNIQUE (nome)
);

ALTER TABLE BoaIniciativa.Material OWNER TO boaini;

INSERT INTO BoaIniciativa.Material (codMaterial,nome,medida) Values (0,'Dinheiro','R$');
