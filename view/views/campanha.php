  ...
  <div class="row">
    ...
    <div class="row">
      ...
      <?php  if(!($campanha->getFinalizaPorData())){
        ...
        //verifica as metas de uma campanha
        $todasmetas = DoadorController::getInstance()->verificarMeta($campanha->getIdCampanha());

        for ($i = 0; $i < count($todasmetas); $i++) {
          //pega uma das metas
          $meta = $todasmetas[$i];
          //ver qual o material deseja ser arrecadado
          $material = MaterialDAO::getInstance()->buscarMaterial($meta->getCodMaterial());


          ?>
          <div class="col-xs-6">
            <h3>Meta <?php $material->getNome(); ?>:</h3> <?php echo $meta->getQuantidade();?>
          </div>
          <?php }//fecha for
        }//fecha if ?>
      </div><div class="row">
